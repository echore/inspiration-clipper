...
> **2026-07-29 使用范式修正（用户裁决，实测后定）**：一个用户只会配一个目的地，「Notion 装不下自动改存 Obsidian」的跨 app 降级在真实使用中不成立。v1 的 UI 只提供单目的地选择；超限时明确报错并点名目的地与上限（「这个文件 8.2 MB，超过 Notion 的 5 MB 上限，没存上」），不做静默降级。路由层的链式降级保留为内部能力（不在 UI 暴露），供将来多目的地场景（iOS 快捷指令 / 飞书）评估复用。验收标准第 5 条随之作废，改为「超限报错含目的地名与数值」。
...
- **Notion 模板**（用户 duplicate）：预配画廊视图（卡片封面绑 `Image` 属性）、随机排序、约定好的属性结构（`Name` / `Image` / `Source URL` / `Source Title` / `Captured` / `Tags` / `Shuffle`）、以及"建 integration"的分步引导。模板本来就是 Notion 生态最自然的分发方式。

  **随机公式（实测可用，天级分桶）**：

  ```
  let(d, replaceAll(id(), "[^0-9]", ""),
      mod(toNumber(substring(d, length(d) - 6, length(d))) * floor(timestamp(now()) / 86400000), 9973))
  ```

  ⚠️ **踩坑记录**：社区广泛流传的写法是取 `id()` 数字串的**开头**几位（如 YouTube 教程里的 `toNumber(replaceAll(id(), "[^0-9]", ""))`）。**这在本项目场景下必然失效**——同一数据库中批量创建的 Notion 页面 ID 共享很长前缀（如全部为 `3ac942e6a592…`），取头部会让所有行得到同一个值，Shuffle 列变常数、排序失效、静默回落到 ID 顺序。**必须取尾部**（`length(d) - 6`）。而扩展批量写入恰恰就是"批量创建"场景。

  **模板已建成**：位于用户 workspace 的「模板」页下，database id `3ac942e6a59280a5be6ce2122761f02d`，含 Shuffle Gallery 视图。目前有 12 行 `01`–`12` 占位数据待清理。

  **模板可程序化生成**：Notion MCP 连接器支持 `create-database` / `update-data-source` / `create-view`（含 gallery 类型与 `SORT BY` / `COVER` 配置），本模板即由此建成。裸 Notion REST API 不支持建视图，连接器支持——这对模板的分发和版本管理是有利条件。
...
| **Notion** | ✅ | ✅ 画廊视图 | ✅ **已实测**（天级分桶） | **免费版 5MB** |

两者并列，不分先后。

**准入条件已满足（2026-07-28）**：Notion 的第三根支柱（随机重现）原为待验，现已实测通过，转为正式准入。两个目的地均完整满足三根支柱。
...
5. ~~传一个 >5MB 的 GIF 到 Notion 免费版 → **自动降级到 Obsidian，并有明确 toast 说明原因和去向**~~（2026-07-29 作废，见组件 2 的使用范式修正；改为：超限时 toast 明确报错，含目的地名与上限数值，不降级）
...
1. ~~Notion 随机排序能否做到"每次打开重洗"~~ **已解决（2026-07-28 实测）**。可以。见「组件 4」的公式。分钟级分桶实测 130 秒后 12/12 位置全变；改天级分桶后 130 秒 12/12 不变。采用天级——每天首次打开是新顺序、当天内稳定，结构性避免了滚动时卡片跳动。**注意此结论来自服务端视图查询；渲染层（app/web 客户端）的实际重排时机未单独验证，但因天级分桶下一天只变一次，风险已大幅降低。**
2. **是否需要自建查看器** —— **第一版不需要**。Obsidian 和 Notion 两个目的地的随机重现均已验证可用，呈现层可以完全交给目的地。此问题仅在引入**飞书**时重新激活（飞书无 random 函数，见「待定」），届时再评估是否要自带一个浏览器内的随机瀑布流查看器。这会把产品从"捕获器"变成"捕获器 + 查看器"，是重大范围扩张，不应为单个目的地触发
